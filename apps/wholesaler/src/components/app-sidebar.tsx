...
import { LayoutDashboard, Users, Package, ShoppingCart, UserCheck, Handshake, CreditCard, MapPin, Store, LogOut, type LucideIcon } from "lucide-react";
...
export function AppSidebar({ tenantName, userName, userEmail, userAvatar, onLogout }: AppSidebarProps) {
      ...
      <SidebarHeader>
        <div className="flex items-center gap-2 px-2 py-1">
          <div className="flex aspect-square size-8 items-center justify-center rounded-lg bg-primary text-primary-foreground font-bold text-sm">
            {tenantName?.charAt(0) || "P"}
          </div>
          <div className="grid flex-1 text-left text-sm leading-tight">
            <span className="truncate font-semibold">{tenantName || "Portal"}</span>
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton size="lg" onClick={onLogout} className="hover:bg-accent h-auto py-2">
              <Avatar name={userName || "U"} src={userAvatar} size="sm" />
              <div className="grid flex-1 text-left text-sm leading-tight">
